...
def read_cartesian_gt_txt(gt_txt_path):
    """Read Cartesian GT with the same frame-index structure as Polar GT.

    Each non-comment line is:
    ``frame_idx, object_label, x, y, z, x_width, y_width, z_width, yaw_deg, cls``.
    The first field is one-based radar dataset order, just like
    :func:`read_gt_txt`.  Dimensions are full metric dimensions in metres.
    """
    gt_by_file_idx = defaultdict(list)
