...
class ToolPathTests(unittest.TestCase):
    def test_moved_tools_resolve_the_repository_root_correctly(self):
        ...
        self.assertEqual(
            importlib.import_module(
                "tools.figures.model7.architecture"
            ).ROOT,
            project_root,
        )
        self.assertEqual(
            importlib.import_module(
                "tools.analysis.generate_experiment_data_summary"
            ).ROOT,
            project_root,
        )
        self.assertEqual(
            importlib.import_module(
                "tools.analysis.grafic_visualization"
            ).PROJECT_ROOT,
            project_root,
        )
        self.assertEqual(
            importlib.import_module(
                "tools.maintenance.rebuild_domain_shift_tables"
            ).PROJECT_ROOT,
            project_root,
        )
        self.assertEqual(
            importlib.import_module(
                "tools.analysis.generate_group1_domain_shift_summary"
            ).ROOT,
            project_root,
        )
        self.assertEqual(
            importlib.import_module(
                "tools.analysis.plot_weather_road_frames"
            ).PROJECT_ROOT,
            project_root,
        )
    ...
    def test_relocated_analyses_keep_their_original_output_directories(self):
        ...
        weather = importlib.import_module("tools.analysis.plot_weather_road_frames")
        self.assertEqual(
            group1.OUTPUT_DIR,
