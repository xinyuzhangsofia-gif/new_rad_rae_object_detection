...
def make_multisensor_config(**overrides):
    args = load_visualization_config(mode="multisensor", **overrides)
...
class VisualizationCheckpointPredictorTest(unittest.TestCase):
    def test_gt_text_is_configurable_and_disabled_by_default(self):
        self.assertFalse(load_visualization_config().show_gt_texts)
    ...
    def test_ra_gt_label_is_above_and_prediction_label_is_below(self):
        ...
        add_ra_box_label(axis, box, "GT | R | Sedan", "lime")
        add_ra_box_label(axis, box, "Pred | Sedan", "red")
    ...
    def test_camera_overlay_uses_green_gt_and_red_prediction(self):
    ...
    def test_real_checkpoint_predicts_metric_boxes_for_real_current_frame(self):
        ...
        if not (Path(cfg.radar_npy_root) / "11" / "rad" / "00034.npy").is_file():
        ...
        red_prediction_pixels = (
            (image[:, :, 2] > 200)
            & (image[:, :, 1] < 100)
            & (image[:, :, 0] < 100)
        )
        self.assertTrue(green_gt_pixels.any())
        self.assertTrue(red_prediction_pixels.any())
