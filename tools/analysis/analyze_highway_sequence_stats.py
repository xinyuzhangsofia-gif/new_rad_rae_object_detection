...
def count_sequence_objects(sequence: int) -> dict[str, int]:
    ...
    gt_by_file_idx = read_cartesian_gt_txt(str(gt_path))
    ...
    for file_idx, objects in gt_by_file_idx.items():
        ...
        for obj in objects:
            radius, azimuth, elevation = cartesian_to_rae(*obj["box_metric"][:3].tolist())
            raw = {
                "r_idx": (radius - RANGE_AXIS.minimum) / RANGE_AXIS.step,
                "a_idx": (azimuth - AZIMUTH_AXIS.minimum) / AZIMUTH_AXIS.step,
                "e_idx": (elevation - ELEVATION_AXIS.minimum) / ELEVATION_AXIS.step,
            }
