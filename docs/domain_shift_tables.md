...
`sequence_information.csv`. Frame/object statistics are computed from the
current Cartesian-radar per-frame training labels; environment, time-of-day,
and weather metadata retain the original K-Radar statistics-image annotations.

## Configuration

The automatic update is controlled in `configs/evaluation.py`:
