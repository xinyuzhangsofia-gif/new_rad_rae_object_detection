"""Load and validate explicit per-object ignore overrides."""
...
def validate_object_ignore_overrides(
        override_map,
        summary,
        class_to_idx,
        sequence,
        objects_for_file_idx,
    ):
    """Validate that every override selects one evaluable GT object."""
    missing_frames = tuple(summary.get("missing_frame_names", ()))
    if missing_frames:
        raise ValueError(
            "Evaluation object-ignore override contains frames absent from "
            f"sequence {sequence}: {missing_frames[:10]}"
        )

    validated = 0
    for file_idx, requested_labels in override_map.items():
        by_label = {}
        for obj in objects_for_file_idx(int(file_idx)):
        ...
        for object_label in requested_labels:
            ...
            if len(matches) != 1:
                raise ValueError(
                    ...
                    f"sequence={sequence}, file_idx={file_idx}, "
    ...
    expected = int(summary.get("object_override_count", 0))
    if validated != expected:
        raise ValueError(
            "Evaluation object-ignore override validation count mismatch: "
            f"expected={expected}, validated={validated}."
        )
    summary["strict_validation"] = True
    summary["validated_object_count"] = validated
