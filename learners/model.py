    ...
    def train(self, add_temp=False):
        ...
        for epoch in range(self.nb_epochs):
            if epoch % self.lr_step == 0 and epoch != 0:
                scheduler.step()
            for _, sequence_data in enumerate(train_loader):#_means that we do not use the index of the sequence in the loop
                ...
                for _, frame in enumerate(frame_dataloader):
                    ...
                    rd_mask = frame['rd_mask'].to(self.device).float()#mask is the ground truth annotation for the range-doppler map
                    ...
                    if nb_losses < 3:
                        ...
                        rd_loss = torch.mean(torch.stack(rd_losses))
                        ra_losses = [c(ra_outputs, torch.argmax(ra_mask, axis=1))#class index is obtained by argmax of the mask
                                     for c in ra_criterion[:2]]
                        ra_loss = torch.mean(torch.stack(ra_losses))#stack the losses and take the mean to get the final loss for range-angle
                        # Coherence loss
                    ...
                    if iteration % self.loss_step == 0:
                        ...
                        if nb_losses > 2:
                            self.visualizer.update_multi_train_loss(train_loss, rd_train_loss,
                                                                    ...
                                                                    coherence_train_loss)#iteration is the global step for tensorboard visualization
                        else:
                            self.visualizer.update_multi_train_loss(train_loss, rd_train_loss,
                                                                    rd_train_losses, ra_train_loss,
                                                                    ra_train_losses, iteration)
